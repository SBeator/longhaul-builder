...
### 0. 先确认执行档位（★动手前第一件事，硬门）
longhaul 默认 = 上面那套**完整引擎**（lhb new→spec-converge→plan→confirm→run 的 cron 自驱 + 每步独立判官）。但当交付物**不是典型可测代码**（大量散文/文档/配置/提示词，或 spec 已在别处冻结）时，引擎的 TDD红绿/probe/判官循环可能空转——**这时你大概率会想降级成"直接撰写 + 人审"。降级本身往往是对的；错的是偷偷降级。**

所以动手前，按**观察到的事实**判断并行动：
- 交付物主要是「能写自动探针验证的代码」→ 默认走完整引擎，直接进第 1 步。
- 交付物主要是「散文/文档/配置/提示词」或「spec 已冻结」→ **停下，先把判断发给人确认**：①你想用哪种档位（完整引擎／降级成什么）②为什么 ③这样**保留**哪些纪律、**放弃**哪些（如：不跑 cron，但每件产物仍独立判官审一次）。**得人明确同意后再动手。**

**铁律（堵死踩过的坑）：**
- **绝不偷偷换流程**：未经人同意，不得把引擎默默替换成自己的临时流程。"引擎不适用"是一个**要发人确认的决定**，不是你一个人能拍的。
- **绝不把严谨度说大**：凡对外说的流程严谨度（"每步独立判官""TDD""每阶段审"）必须等于你**实际在做**的。只在末尾审一次，就说"末尾审一次"。
- **违背字面 = 违背精神**：用"我借了它的精神/纪律"给"跳过确认"找补，就是违规。

**Red flags（冒出这些念头就停下来发人确认档位，别直接写）：**
- "这项目是散文 / spec 已冻，引擎不适用，我直接写就行" ← 判断多半对，但要先发人确认，不是直接动手
- "我借 longhaul 的精神、跳过 plan/run 循环" ← 跳过前先确认
- "我每阶段都会过 Codex 审"（实则只打算末尾审一次） ← 说做不一致，先把实际档位说清

### 1. 老化 / 把需求聊清 + 双 agent 把 spec 钉硬（★第 1 个必停：P0）—— 你唯一要和人深聊的地方
这一步是 **Claude（你，主笔）+ Codex（独立审）协作**，不是你一个人闷头出 spec——前期多拉个 Codex 较真，省后面整轮走偏（spec 错在这里后面全返工）。按这个流程走：
1. **双 agent 澄清提问**：你苏格拉底式分析需求、列要问人的问题；**同时让 Codex 独立分析同一段需求各自出问题**（用 codex 绑定起一个独立 Codex 分析即可）；你**把两边问题汇总成一份清单**问人。直到要做成什么、给谁用、什么算"做完"（验收标准 + 每条怎么验=探针）、范围边界、可逆假设都问清。
2. **你和 Codex 都确认无遗留疑问** → 进 spec 设计。
3. **你写 spec 初稿**：`lhb new <项目目录> "<一句话>"` 生成骨架，把 `spec.md` 填实（User Stories / **设计·架构** / 可测 AC / 验收探针 / P0·P1·P2 / Assumptions），`[NEEDS CLARIFICATION]` 一条条问掉。把**澄清问答 + 需求背景写进 `<dir>/.longhaul/p0-context.md`**（spec-converge 时给双方当背景）。
   - **★把"设计/架构"冻进 spec（ai-cockpit 复盘补的关键）**：确认过的设计稿/架构图、视觉/形态意图、关键取舍（哪些原生做深、哪些先 iframe 嵌）都写进 spec 的「## 设计 / 架构」节，别只停在聊天里（否则 loop 只对文字 spec 建、成品会偏离设计）。"半数模块是 iframe 嵌旧站"这类取舍必须当面讲清 + 写进 spec。门2 reviewer 会据这节核对成品是否忠于设计。
4. **spec 双 agent 互审收敛**：`lhb spec-converge <dir>` —— **Codex 审 spec → 你（Claude）按反馈就地改 → Codex 再审**，循环到两人一致；**最多 3 轮**还不一致就把分歧点整理出来给人裁决（不无限 ping-pong）。结果 + **讨论了几轮**落在 `<dir>/.longhaul/spec-converge.json`。
5. **把最终 spec 发人确认**：只给人结果（最终 spec + 一句「Claude 和 Codex 讨论了 N 轮达成一致」），中间来回不打扰人。
6. **人说有问题** → 把人的意见补进 `p0-context.md`、重跑 `lhb spec-converge`，收敛后再发人——直到人点头。
7. 人点头 → 进第 2 步拆 milestone。

关键：**别急着往下**，一直到人 + Codex 都认 spec 够硬再走。
...
- **缺省 = 分阶段异构互审**：出方案=Claude / 审方案=Codex / 实施=Codex / 审实施=Claude（每步都另一方审；codex 没装该步退回 claude）。
- **无自定义诉求的项目不需要任何配置**——`lhb new` 出来的项目没有 agents.env，直接吃全局默认。`agents.env` 是**纯覆盖文件**，只在你要给某项目配一套和全局不一样的角色时才有。
- 改某阶段：`lhb agents <dir> --plan c|x --plan-review c|x --impl c|x --impl-review c|x`（旧式 `--driver/--judge` 仍支持）。**选的就是默认时不写文件**（避免把当时的默认焊死进项目变陈旧）；在有旧配置的项目上选默认会**清掉**旧 agents.env、回到纯吃全局默认。
...
- **分阶段配不同 agent（#10a）**：通用槽之上可按阶段覆盖——`LONGHAUL_DRIVER_CMD__plan` / `__impl`、`LONGHAUL_JUDGE_CMD__plan_review` / `__impl_review`（分阶段槽最优先，没配回落通用槽，完全向后兼容）。即「出方案 / 实施 各配一个 driver、审方案 / 审实施 各配一个 judge」。
- **plan 多 agent 协同 panel（#10b）**：`LONGHAUL_PLAN_PANEL="cmdA ||| cmdB ||| cmdC"`（`|||` 分隔）→ 出方案后由 **N 个独立 reviewer 一起审**、聚合裁定（**任一要求打回即打回**，最大化在 plan 期抓走偏；全数通过才放行、合并各家条件）。每个 panelist 留审计文件 `evidence/<mid>/review-plan_review.panel-<i>.json`。配 ≥2 个才成 panel，否则回落 #10a 单审。也是「plan 多 agent 是否更好」A/B 课题的实验开关。
- **测试独立 agent（课题，可选）**：`LONGHAUL_TEST_CMD="..."`（或 `LONGHAUL_DRIVER_CMD__test`）→ impl 之后让一个**独立 agent** 写+跑该 milestone 的验收测试（与实现者解耦、不能给自己放水），证据落 `evidence/<mid>/test-agent.txt`，门2 judge 一并审。**不配=老行为**（实现者自己 TDD）。是「测试单独 agent 是否更好」A/B 课题的实验开关；非阻塞（test agent 失败只记一笔，验收仍由探针+门2 把关）。
...
lhb spec-converge <dir> [--max-rounds 3]             # spec 双 agent 收敛（Codex审↔Claude改，到一致/升级；结果含"几轮"）
