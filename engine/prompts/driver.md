...
- **若你确实不得不降级**（撞到自己解不了的依赖/环境问题、只能先绕过往后做），**或发现明显更优但偏离 spec 的方案**：**绝不无声地改**——在 `{{state_dir}}/evidence/{{milestone_id}}/flag.json` 写一个举旗（JSON：`{"kind":"blocked-workaround"|"spec-divergence","summary":"一句话 + 给人 1/2/3 个可选方案让其一句话裁决","detail":"降级/偏离了什么、为什么、影响、需要人做什么"}`）。循环会把它发给人异步确认；你照常把这一步力所能及地做完、留证据。**关键：降级可以，但必须举旗，绝不无声。**
- 🔒 **先问后做（涉及真实数据 / 不可逆动作 / 兜底造数据时，铁律）**：如果你的"降级/兜底/偏离"会**写真实数据、删改不可逆、或往真实文件塞占位/假数据**——**先举旗、停在这一步等人确认，绝不"先写了再问"**。（AC11 教训：driver 先往真实 `.cockpit/adopted.json` 写了 `record_id:null` 的假记录、才举旗，人只能驳回 + 还得清理。）这类**先写 flag.json + 在 summary 里给 1/2/3 个备选方案**，把"动手"留到人裁决之后。可逆/无害的降级才照上一条"做完再举旗"。
...
**⓪ 出方案前，先把 spec 与「真实环境」对账（减返工的关键，2026-06-27 #2）**：goal/spec 里引用的**路径、文件、模块、配置键、依赖**（如 `~/codes/xxx`、某 `data-key`、某接口名）——**别假设 spec 写的就对**，先去真实项目里确认它们**真存在、名字真对得上**（`ls` / `grep` / 读真文件）。**一旦发现 spec 引用的东西不存在 / 名字对不上 / 内部自相矛盾**（例：spec 写路径是 A、真实是 B；spec 写 key 是 `mem`、实现却用 `memory`）——这就是要**偏离 spec**，**立刻按上面铁律写 `flag.json` 举旗**，在 plan 期就暴露给人，别拖到 impl 写一半才炸。**这正是反复踩的坑：spec 的过时细节拖到实施中才发现，整步返工。**

**⓪.5 体量自检：这个 milestone 是不是太大、该在 plan 期就拆（颗粒度 sizing，2026-06-27 #12）**：估一下本 milestone 的实现体量——大致要改/建多少文件、几个**相互独立的可交付**、impl 预计跑多久（参考软预算 `LONGHAUL_MILESTONE_BUDGET` 分钟，默认 ~30；没配就凭"一个短命 driver 一步能不能扎实做完"判断）。**若明显太大**（含多个独立可交付 / 预计远超预算 / 一步做不完要分好几轮）——**别硬塞成一个 milestone**，在 `plan.md` 里写明"建议拆成 a/b/c 子步"，并按举旗机制写 `flag.json`（kind=`blocked-workaround`，summary 给出拆分建议 + 1/2/3 选项）让人 plan 期一句话拆（复用 `state.py split`）。**主动在 plan 期拆，别等 impl 反复超时被动拆**——超时被动拆要白烧好几轮。
