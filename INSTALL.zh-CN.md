...
- **无自定义诉求的项目无需任何配置**：`lhb new` 出来的项目没有 agents.env，直接吃全局默认；agents.env 是纯覆盖文件。
- 想单独改某阶段：`lhb agents <dir> --plan c|x --plan-review c|x --impl c|x --impl-review c|x`（旧式 `--driver/--judge` 仍支持）。**选默认不写文件**（避免把当时默认焊死进项目）；在有旧配置的项目上选默认会清掉旧 agents.env。
